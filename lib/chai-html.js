  ...
  function compare (_super) {
    return function (value) {
      if (utils.flag(this, 'html')) {
        ...
        let diff = deepDiff(lhsTree, rhsTree, (path, key) => key === '__location')
        ...
        if (diff) {
          ...
          // serialize normalized tree into HTML string
          lhsHtml = parse5.serialize(lhsTree)
          rhsHtml = parse5.serialize(rhsTree)

          // extract changed node from normalized tree
          const lhsNode = walk(lhsTree, diff.path)
          const rhsNode = walk(rhsTree, diff.path)

          // get string indices based on whether it's an addition, edit or deletion
          const indices = strategy(diff, lhsNode, rhsNode)

          lhsHtml = extract(lhsHtml, indices.lhs)
          rhsHtml = extract(rhsHtml, indices.rhs)
        }
